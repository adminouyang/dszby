...
def load_config():
    ...
    default_config = {
        'fofa': {
            'email_base64': '',  # Base64编码的邮箱
            'api_key_base64': '',  # Base64编码的API KEY
        },
        'settings': {
            'speed_threshold': '0.1',
            'max_workers': '5',
    ...
    if not os.path.exists(CONFIG_FILE):
        ...
        with open(CONFIG_FILE, 'w', encoding='utf-8') as f:
            config.write(f)
        print(f"⚠️ 配置文件不存在，已创建默认配置文件: {CONFIG_FILE}")
        print("💡 请编辑配置文件并设置Base64编码的FOFA邮箱和API KEY")
        return None
...
def get_fofa_cookie():
    """从cookie.txt文件读取FOFA Cookie"""
    if os.path.exists(COOKIE_FILE):
        try:
            with open(COOKIE_FILE, 'r', encoding='utf-8') as f:
                cookie_content = f.read().strip()
                if cookie_content:
                    print("✅ 已从cookie.txt加载Cookie")
                    return cookie_content
        except Exception as e:
            print(f"❌ 读取cookie.txt失败: {e}")
    
    print("⚠️ cookie.txt文件不存在或为空")
    return ""
...
def crawl_fofa_with_api():
    ...
    print(f"🔍 开始使用FOFA API爬取，共 {len(SEARCH_QUERIES)} 个搜索查询")
    ...
    def search_single_query(query):
        ...
        try:
            ...
            encoded_query = encode_query(query)
            url = f"https://fofa.info/api/v1/search/all?qbase64={encoded_query}&page=1&size=100&fields=ip,port"
            
            headers = get_api_headers()
            response = requests.get(url, headers=headers, timeout=timeout)
            
            if response.status_code == 401:
                print(f"❌ API认证失败，将尝试使用Cookie方式")
                return set()
            elif response.status_code == 402:
                print(f"❌ 账户余额不足，将尝试使用Cookie方式")
                return set()
            ...
            if not data.get("error"):
                ...
                print(f"✅ 查询 '{query}' 获取到 {len(page_ips)} 个IP")
                return page_ips
            else:
                print(f"❌ 查询 '{query}' 失败: {data.get('errmsg', '未知错误')}")
                return set()
                
        except Exception as e:
            print(f"❌ 查询 '{query}' 失败: {str(e)[:100]}")
            return set()
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=max_workers) as executor:
        ...
        for future in concurrent.futures.as_completed(futures):
            ...
            try:
                ...
                print(f"📊 当前IP总数: {len(all_ips)}")
            except Exception as e:
                print(f"❌ 处理查询 '{query}' 时发生错误: {e}")
    
    print(f"🎯 FOFA API爬取完成，总共获取到 {len(all_ips)} 个有效IP")
    return list(all_ips)
...
def crawl_fofa_with_cookie():
    ...
    print(f"🔍 开始使用Cookie爬取FOFA，共 {len(urls)} 个搜索页面")
    ...
    for i, url in enumerate(urls, 1):
        ...
        try:
            ...
            headers = get_api_headers()  # 使用包含Cookie的headers
    ...
    print(f"🎯 Cookie爬取完成，总共获取到 {len(all_ips)} 个有效IP")
    return list(all_ips)

def crawl_fofa_ips():
    """爬取FOFA IP，优先使用API，失败则使用Cookie"""
    print("🚀 开始爬取FOFA IP...")
...
def generate_iptv_txt(channels_dict):
    ...
    try:
        with open(output_file, 'w', encoding='utf-8', errors='ignore') as f:
            update_time = datetime.now().strftime('%Y/%m/%d %H:%M更新')
