...
def timeout(seconds):
    ...
    if hasattr(signal, 'SIGALRM'):
        ...
        try:
            ...
            signal.alarm(0)
            signal.signal(signal.Signal, old_handler)
    else:
        # Windows系统使用线程实现超时
        def target():
            time.sleep(seconds)
        ...
        timer = threading.Thread(target=target)
        timer.daemon = True
        timer.start()
        try:
            yield
...
def requests_with_timeout(url, timeout_seconds=5, **kwargs):
    """带超时的requests请求（替换eventlet超时）"""
    try:
...
def get_ip_info(ip_port):
    ...
    try:
        ...
        # 使用IP-API查询
        try:
            response = requests_with_timeout(f"http://ip-api.com/json/{ip}?lang=zh-CN", timeout=5)
...
def test_ip_availability(ip_port):
    ...
    try:
        ...
        response = requests_with_timeout(json_url, timeout=5)
...
def test_channel_speed(channel_url, max_attempts=2):
    ...
    for attempt in range(max_attempts):
        try:
            # 获取m3u8文件内容
            response = requests_with_timeout(channel_url, timeout=3)
            if response.status_code != 200:
            ...
            try:
                # 使用新的超时控制替换eventlet
                with timeout(5):  # 5秒超时
                    ts_response = requests_with_timeout(ts_url, timeout=6, stream=True)
